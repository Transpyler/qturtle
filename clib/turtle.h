/**
 * This is for teachers making the jump from Python to C. Just #include<turtle.h>
 * and make QTurtle commands available in C.
 *
...
/**
 * Ensure that the interpreter has been correctly initialized.
 * Can be called multiple times, but all calls after the first are no-ops.
 */
void _start_interpreter(void) {
    if (_PY_STARTED == 0) {
        ...
        PyRun_SimpleString("import tugalib as _tugalib\n");
...
/**
 * Call a Python function in global namespace using formatted variable specifier
 * to set the type of each argument.
 *
 * This function just eval a string within python's interpreter.
 */
...
long ipycall(char* cmd, ...) {
    va_list argptr;
    _va_pycall(cmd, argptr);

    // Get output
    PyObject *pyobj = PyObject_GetAttrString(_PY_MAIN, "__result__");
    long result = PyInt_AsLong(pyobj);
    Py_DecRef(pyobj);
    return result;
}

double fpycall(char* cmd, ...) {
    va_list argptr;
    _va_pycall(cmd, argptr);
